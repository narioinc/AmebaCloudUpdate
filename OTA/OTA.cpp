...
int OTAClass::beginCloud(char *ipstring, uint16_t port, bool reboot_when_success) {
    ...
    do {
        ...
        localHost.sin_addr.s_addr = inet_addr(ipstring);

		OTA_PRINTF("Try to connect...%s \r\n", ipstring);
        if ( lwip_connect(client_socket, (struct sockaddr *)&localHost, sizeof(localHost)) < 0 ) {
            OTA_PRINTF("ERR: Connect error\r\n");
            break;
        }
